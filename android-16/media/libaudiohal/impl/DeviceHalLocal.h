...
class DeviceHalLocal : public DeviceHalInterface
{
  public:
    status_t getAudioPorts(
            std::vector<media::audio::common::AudioPort> *ports) override;

    status_t getAudioRoutes(std::vector<media::AudioRoute> *routes) override;

    status_t getSupportedModes(
            std::vector<media::audio::common::AudioMode> *modes) override;
    ...
    status_t getInputBufferSize(
            struct audio_config *config, size_t *size) override;
    ...
    // List microphones
    status_t getMicrophones(
            std::vector<audio_microphone_characteristic_t> *microphones) override;
    ...
    status_t prepareToDisconnectExternalDevice(
            const struct audio_port_v7 *port) override;
